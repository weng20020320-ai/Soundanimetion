...
| Web demo | https://soundanimetion.vercel.app（Vercel 默认子域，已上线） |
...
```json
{
  ...
  "demoUrl": "https://soundanimetion.vercel.app",
...
Wavelet 的 Web demo 部署在**独立的 Vercel 项目**（不是主页项目），URL 是 **https://soundanimetion.vercel.app** 。

- 用户决定**不绑自定义子域名**，直接用 Vercel 默认 `.vercel.app` URL
- `apparatus.json.demoUrl` 已对应更新，主页 git pull 一次就读到
- Vercel 项目已 Import、build 已绿、URL 已稳定可访问
- 同源仓库每次推 main，Vercel 自动重新部署

主页 Cursor 这边**不需要做任何 DNS / Vercel 工作**。"Try online" 按钮直接指向 `demoUrl` 即可。
...
| `.github/workflows/release.yml`（自动构建） | ✓ `v0.2.1` tag 已推，构建跑过、Releases 页面有产物 | 主页下载按钮直接生效 |
| 卡片封面组件 `docs/handoff/wavelet-cover.tsx` | ✓ 已就绪 | 取代静态截图，主页 agent 直接复制使用 |
| Vercel 部署（Soundanimetion 项目） | ✓ 已上线 https://soundanimetion.vercel.app | 用户决定用 Vercel 默认子域，不绑自定义域名 |
| DNS 自定义子域 | ❌ **不做**（用户决定保持现状） | 不影响 demoUrl 可用性 |
